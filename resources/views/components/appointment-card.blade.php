...
<div class="absolute z-10 flex flex-col items-center justify-center text-white rounded-lg shadow-md border border-white/20 p-1 text-center overflow-hidden"
    ...
    <a href="{{ route('appointments.show', $appointment) }}" class="absolute inset-0 z-20">
        <p class="font-bold uppercase truncate w-full">{{ $appointment->client }}</p>
        <p class="opacity-80">{{ $appointment->service ?? 'Não informado' }}</p></p>
        <p class="opacity-80">{{ $start->format('H:i') }} - {{ $end->format('H:i') }}</p>
    </a>
</div>